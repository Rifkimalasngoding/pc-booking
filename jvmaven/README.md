...
3. Sesuaikan `src/main/resources/application.properties` kalau username/password MySQL kamu beda dari default. Database & tabel dibuat **otomatis** saat aplikasi start.

4. Jalankan dari folder `pcbooking`:
   ```bash
...
- Ganti `username` / `password` sesuai kredensial MySQL/MariaDB kamu.
...
| `Access denied for user 'root'@'localhost'` | Username/password di `application.properties` tidak cocok dengan MySQL kamu |
...
- Sesi login masih ditangani manual lewat `HttpSession`, belum pakai filter/proteksi endpoint dari Spring Security penuh. Lihat `NOTES.md` untuk daftar hal yang belum digarap kalau mau dikembangkan lebih lanjut (mis. guard session di tiap controller, validasi input).
